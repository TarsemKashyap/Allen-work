...
public class reject_mytask : fbs_base_page, IRequiresSessionState
{
  ...
  protected new void Page_Load(object sender, EventArgs e)
  {
    try
    {
      if (!string.IsNullOrWhiteSpace(this.Request.QueryString["workflow_id"]))
        this.hidWorkflowID.Value = this.Request.QueryString["workflow_id"];
      else
  ...
  protected void btn_reject_Click(object sender, EventArgs e)
  {
    try
    {
      ...
      workflow workflow = this.workflows.get_workflow(Convert.ToInt64(this.hidWorkflowID.Value), this.current_user.account_id);
      workflow.action_status = api_constants.workflow_action_status["Rejected"];
      ...
      workflow work = this.workflows.update_workflow(workflow);
      if (work.workflow_id > 0L)
      {
        if (work.action_type == (short) 3 || work.action_type == (short) 2)
        {
          this.update_booking_info(work.reference_id, api_constants.booking_status["Rejected"], work.action_remarks);
      ...
      if (!string.IsNullOrWhiteSpace(this.Request.QueryString["tab"]))
      {
        ...
        {
          this.hidtab.Value = this.Request.QueryString["tab"].ToString();
          this.Page.ClientScript.RegisterStartupScript(this.GetType(), "MyFun1", "Mytask();", true);
        }
      }
      else
        this.Response.Redirect("reject_mytask.aspx", true);
    }
    catch (Exception ex)
    {
      fbs_base_page.log.Error((object) "Error -> ", ex);
  ...
  protected void Button1_Click(object sender, EventArgs e) => Modal.Close((Page) this);
}
