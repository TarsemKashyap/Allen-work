...
// Type: controls_asset_layout_image
...
public class controls_asset_layout_image : fbs_base_user_control
{
  protected Literal lblError;
  protected HtmlGenericControl alertInfo;
  protected Literal lit_msg;
  protected System.Web.UI.WebControls.Image layoutImg;
  protected HtmlGenericControl imgdiv;
  protected FileUpload upload_layout;
    ...
    try
    {
      ...
      if (this.Request.QueryString["asset_id"] != "0")
      {
        foreach (long key1 in this.objAsset.asset_properties.Keys)
        {
          ...
          if (assetProperty.property_name == "layout_image" && assetProperty.property_value != "" && assetProperty.property_value != "")
          {
            ...
            foreach (long key2 in this.objAsset.documents.Keys)
            {
              ...
              if (document.document_id == int64)
              {
                ...
                this.layoutImg.ImageUrl = "data:image/png;base64," + Convert.ToBase64String(binaryData, 0, binaryData.Length);
              }
            }
          }
        }
      }
      if (this.Session["invalid_layout_size"] == null)
        return;
      if (!string.IsNullOrWhiteSpace(this.Session["invalid_layout_size"].ToString()))
      {
        ...
        this.Session.Remove("invalid_layout_size");
        this.Session.Remove("W");
        this.Session.Remove("H");
        this.Session.Remove("SZ");
        this.Session["Active_tab"] = (object) "tab_layout";
      }
      else
  ...
  protected void btn_submit_Click(object sender, EventArgs e)
  {
    try
    {
      ...
      if (!this.upload_layout.HasFile)
        return;
      int contentLength = this.upload_layout.PostedFile.ContentLength;
      byte[] numArray = new byte[contentLength];
      byte[] fileBytes = this.upload_layout.FileBytes;
      ...
      int int32_1 = Convert.ToInt32(dataSet.Tables[0].Select("parameter='layout_width'")[0]["value"].ToString());
      int int32_2 = Convert.ToInt32(dataSet.Tables[0].Select("parameter='layout_height'")[0]["value"].ToString());
      int int32_3 = Convert.ToInt32(dataSet.Tables[0].Select("parameter='layout_size'")[0]["value"].ToString());
      if (image.Width <= int32_1 && image.Height <= int32_2 && contentLength <= int32_3)
      {
        ...
        foreach (long key in this.objAsset.asset_properties.Keys)
        {
          ...
          if (assetProperty2.property_name == "layout_image")
          {
        ...
        assetDocument1.document_name = this.upload_layout.FileName;
        assetDocument1.document_size = contentLength;
        assetDocument1.document_type = this.upload_layout.PostedFile.ContentType;
        ...
        assetProperty1.property_name = "layout_image";
        ...
        this.layoutImg.ImageUrl = "data:image/png;base64," + Convert.ToBase64String(fileBytes, 0, fileBytes.Length);
        this.Response.Redirect("asset_form.aspx?asset_id=" + assetDocument2.asset_id.ToString() + "#tab_layout", true);
      }
      else
      {
        this.Session["invalid_layout_size"] = (object) "1";
        this.Session["H"] = (object) int32_2;
        this.Session["W"] = (object) int32_1;
        this.Session["SZ"] = (object) (Convert.ToInt32(int32_3) / 1024);
        this.Response.Redirect("asset_form.aspx?asset_id=" + this.Request.QueryString["asset_id"] + "#tab_layout", true);
      }
    }
    catch (Exception ex)
    {
  ...
  protected void btn_remove_Click(object sender, EventArgs e)
  {
    try
    {
      ...
      this.layoutImg.ImageUrl = "~/assets/img/noimage.gif";
      this.Response.Redirect("asset_form.aspx?asset_id=" + assetId.ToString() + "#tab_layout", true);
    }
    catch (Exception ex)
    {
      this.log.Error((object) "Error ->", ex);
