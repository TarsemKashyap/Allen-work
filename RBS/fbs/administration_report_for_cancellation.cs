...
// Type: administration_report_for_cancellation
...
public class administration_report_for_cancellation : fbs_base_page, IRequiresSessionState
{
  ...
  protected void populate_ui(string fromdate, string todate)
  {
    try
    {
      ...
      DataSet dataSet = this.reportings.cancellation_report(fromdate, todate, this.current_user.account_id, groupids);
      ...
      stringBuilder.Append("<th class='hidden-480'>No.of Cancellations</th>");
      stringBuilder.Append("<th class='hidden-480'>% of Cancellation</th>");
      stringBuilder.Append("</tr>");
      stringBuilder.Append("</thead>");
      ...
      for (int index = 0; index < dataSet.Tables[0].Rows.Count; ++index)
      {
        ...
        double num1 = Convert.ToDouble(row["NoOfBooking"].ToString());
        double num2 = Convert.ToDouble(row["SumOfCancel"].ToString());
        string str3 = num2 > num1 ? ">100" : Convert.ToDouble(Convert.ToDouble(num2 / num1) * 100.0).ToString("#.##");
        if (index == 0)
        {
          stringBuilder.Append("<tr class='division'>");
          if (row["Division"].ToString().Trim() == "")
            stringBuilder.AppendFormat("<td>NA</td>");
          else
          ...
          stringBuilder.AppendFormat("<td>{0}</td>", (object) row["SumOfCancel"].ToString());
          stringBuilder.AppendFormat("<td>{0}</td>", (object) str3);
          stringBuilder.Append("</tr>");
          str1 = row["Division"].ToString();
          this.hyperlink_detail_view.NavigateUrl = "requestor_info_from_summary.aspx?fromdate=" + Convert.ToDateTime(fromdate).ToString(api_constants.sql_datetime_format_short) + "&todate=" + Convert.ToDateTime(todate).ToString(api_constants.sql_datetime_format_short) + "&status=" + this.bookings.get_status("Cancelled").ToString();
        }
      ...
      for (int index = 0; index < dataSet.Tables[1].Rows.Count; ++index)
      {
        ...
        stringBuilder.Append("<tr>");
        stringBuilder.AppendFormat("<td>{0}</td>", (object) row["name"].ToString());
        stringBuilder.AppendFormat("<td>{0}</td>", (object) row["NoOfCancellation"].ToString());
        stringBuilder.Append("</tr>");
      }
      if (dataSet.Tables[0].Rows.Count > 0)
      {
        stringBuilder.Append("<tr class='departmenttotal'>");
        stringBuilder.Append("<td></td>");
        stringBuilder.AppendFormat("<td>{0}</td>", (object) dataSet.Tables[0].Rows[dataSet.Tables[0].Rows.Count - 1]["SumOfCancel"].ToString());
        stringBuilder.Append("</tr>");
      }
      stringBuilder.Append("</tbody>");
      stringBuilder.Append("</table>");
    ...
    {
      fbs_base_page.log.Error((object) "Cancellation Reporting Error: ->", ex);
    }
  }
  ...
  protected void btnExportExcel_Click(object sender, EventArgs e)
  {
    try
    {
      ...
      string groupids = this.gp.isAdminType ? "" : "asset_owner_group_id IN (" + string.Join(",", this.gp.group_ids.ToArray()) + ") AND ";
      DataSet ds = this.reportings.cancellation_report(fromdate, todate, this.current_user.account_id, groupids);
      if (ds != null)
      {
      ...
      StringBuilder stringBuilder = new StringBuilder();
      this.Response.Clear();
      this.Response.AddHeader("content-disposition", "attachment;filename=Cancellation_report_" + this.current_timestamp.ToString(api_constants.display_datetime_format) + ".xls");
      ...
      stringBuilder.Append("<tbody>");
      stringBuilder.Append("<tr>");
      stringBuilder.Append("<td style='font-size:24pt;' colspan='3'>Cancellation Report</td>");
      stringBuilder.Append("</tr>");
      stringBuilder.Append("</tbody>");
      stringBuilder.Append("</table>");
      stringBuilder.Append("<table class='table table_summary_report table-bordered' id='list_table_report' style='font-size:11pt;' border='1'>");
      ...
      for (int index = 0; index < ds.Tables[0].Rows.Count; ++index)
      {
        ...
        double num1 = Convert.ToDouble(row["NoOfBooking"].ToString());
        double num2 = Convert.ToDouble(row["SumOfCancel"].ToString());
        string str5 = num2 > num1 ? ">100" : Convert.ToDouble(Convert.ToDouble(num2 / num1) * 100.0).ToString("#.##");
        if (index == 0)
        {
      ...
      if (ds.Tables[0].Rows.Count > 0)
      {
        ...
        stringBuilder.Append("<td></td>");
        stringBuilder.Append("<td></td>");
        stringBuilder.AppendFormat("<td>{0}</td>", (object) ds.Tables[0].Rows[ds.Tables[0].Rows.Count - 1]["SumOfCancel"].ToString());
        stringBuilder.Append("</tr>");
      }
      stringBuilder.Append("</tbody>");
      stringBuilder.Append("</table>");
    ...
    {
      fbs_base_page.log.Error((object) "Export Cancellation Reporting Error: ->", ex);
    }
  }
