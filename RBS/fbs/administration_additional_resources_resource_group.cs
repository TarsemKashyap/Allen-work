...
// Type: administration_additional_resources_resource_groups
...
public class administration_additional_resources_resource_groups : 
  fbs_base_page,
  IRequiresSessionState
{
  ...
  private void populate_ui(DataSet data)
  {
    try
    {
      ...
      this.assets.view_assets(this.current_user.account_id);
      StringBuilder stringBuilder = new StringBuilder();
      ...
      foreach (DataRow row1 in (InternalDataCollectionBase) data.Tables[0].Rows)
      {
        ...
        if (row1["group_name"].ToString().ToUpper() != api_constants.all_users_text.ToUpper())
        {
          ...
          stringBuilder.AppendFormat("<li><a href='resource_group_permissions.aspx?id={0}'><i class='icon-pencil'></i> Edit Permissions</a></li>", (object) row1["group_id"].ToString());
